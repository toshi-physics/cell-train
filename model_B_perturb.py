...
class integrator:
    ...
    def perturb(self, t, x, p):
        #given the perturbation as a function of time, use it 
        self.C[0]=p; # fix time dependent displacement of first end
        n = self.A*x + self.C
        n[self.N-1]*=0
        nn = np.zeros_like(n); nn[2*self.N+1:] += self.b*np.tanh((x[1:self.N]-x[0:self.N-1]-self.lh)/self.lh)/self.t3
        nn[2*self.N] += self.b*np.tanh((x[0]-p-self.lh)/self.lh)/self.t3
        nn[N:2*self.N] -= self.a*np.tanh(x[2*self.N:])/self.t2
        return n+nn

    def rk4step(self, f, t, x, dt, p):
        #assuming the perturbation is the same for the timestep dt and dt/2
        k1 = f( t, x, p)
    ...
    def integrate(self, t, dt, dt_dump, x_init, perturbation, p):
        f = self.perturb
        ndump = np.round(dt_dump/dt)
        dumpSol = np.array([x_init])
        prevSol = x_init
        for i in np.arange(0,t.size-1,1):
            Sol = self.rk4step(f, t[i], prevSol, dt, p[i])
            if i%ndump == 0:
...
for N in [ 20, 30, 50, 75, 100, 120, 150]:
    for omega0 in np.arange( 0.01, 0.26, 0.005):
        print(N, omega0, a*b)
        print(N, a*b)
        ai=a
        abi = ai*b
        bi = b
        quart=bi
        Dr0 = -0.5*N/20    #make extension proportional to size of the train
        
        if perturbation=='step_strain':
            #pick a prev sol for starting case
            ...
            xinit = xinitall[-1,1:]
            quart = -Dr0 #fourth argument in saving datafiles

            p = np.ones_like(t)*Dr0
            p_dump = np.ones_like(t_dump)*Dr0
        elif perturbation=='osc_strain':
            #pick a prev sol for starting state
            xinitall = np.fromfile("steady_state/{:d}/{:1.1f}/{:1.1f}/{:1.1f}/solution_{:1.1f}.dat".format(N, abi, t2, t3, bi))
            ...
            #pick the form of perturbation
            p=Dr0*np.sin(omega0*t)
            p_dump=Dr0*np.sin(omega0*t_dump)

            quart = omega0
        elif perturbation=='square_wave_strain':
            #pick a prev sol for starting state
            xinitall = np.fromfile("steady_state/{:d}/{:1.1f}/{:1.1f}/{:1.1f}/solution_{:1.1f}.dat".format(N, abi, t2, t3, bi))
            xinitall = xinitall.reshape(int(len(xinitall)/(3*N +1)),3*N+1)
            startpos = -int(2000*np.random.rand(1))
            ...
            #generate square wave with frequency omega_0 and duty cycle =0.5
            p = Dr0*np.sin(omega0*t)
            p = np.where(p>0, 1, 0)
            p_dump = Dr0*np.sin(omega0*t_dump)
            p_dump = np.where(p_dump>0, 1, 0)
            ...
            p     = np.zeros_like(t)
            p_dump= np.zeros_like(t_dump)

        I = integrator(N, ai, bi, t2, t3, Dr0, lh, omega0)
        Solution = I.integrate(t, dt, dt_dump, xinit, perturbation, p)
        print(np.shape(Solution), np.shape(p_dump))
        Solution = np.insert(Solution, 0, p_dump, axis=1)
        ...
        if os.path.isfile('{:s}/{:d}/{:1.1f}/{:1.1f}/{:1.1f}/solution_{:1.3f}.dat'.format(perturbation,N ,abi, t2, t3, quart)):
            ...
            Solution.tofile('{:s}/{:d}/{:1.1f}/{:1.1f}/{:1.1f}/solution_{:1.3f}_3.dat'.format(perturbation,N ,abi, t2, t3, quart))
        else:
